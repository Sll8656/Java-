...
public Result login(LoginFormDTO loginForm, HttpSession session) {
    ...
    //这里用到了高级抽象query().eq().one()，不加one的话只构造查询条件，不执行查询。加one就查询了。
...
public Result login(LoginFormDTO loginForm, HttpSession session) {
    ...
    if (!loginForm.getCode().equals(cacheCode)) {
        return Result.fail("验证码不一致，错误~");
    }

    User user = query().eq("phone", phone).one();

    //这个是存入数据库,和后面的存入redis没关系,不管user是不是null都要往redis存
    if (user == null) {
        user = new User();
	...
	//在redis中,key是token,值是哈希表类型的UserDTO
    String token = UUID.randomUUID().toString();

    UserDTO userDTO = BeanUtil.copyProperties(user, UserDTO.class);
    //map的key是属性名称，value是属性值
    Map<String, Object> userMap = BeanUtil.beanToMap(userDTO, new HashMap<>(),
                                                     CopyOptions.create()
                                                     .setIgnoreNullValue(true)
                                                     //类型为long，没法放入redis中。所以将值的类型都toString()
                                                     //这样map的value的类型就全部都是String了
                                                     .setFieldValueEditor((fieldName, fieldValue) -> fieldValue.toString()));


    stringRedisTemplate.opsForHash().putAll( "login:token:" + token,userMap);

    stringRedisTemplate.expire( "login:token:" + token, 360000L, TimeUnit.MINUTES);
...
在这个方案中，他确实可以使用对应路径的拦截，同时刷新登录token令牌的存活时间，但是现在这个拦截器他只是拦截需要被拦截的路径，**假设当前用户访问了一些不需要拦截的路径，比如查看首页不需要登录。那么这个拦截器就不会生效**，**此时令牌刷新的动作实际上就不会执行，**所以这个方案他是存在问题的 
...
因为loginIntercepter不受spring管理，所以如果他要用redis，就不能用@Autowired。要通过构造函数来初始化
...
public class MvcConfig implements WebMvcConfigurer {
    ...
    //order排序表示先经过Refresh，再经过LoginInterceptor
}
...
public Result queryList() {
    String shopTypeJSONList = stringRedisTemplate.opsForValue().get("cache:shopType:");
    if(!StrUtil.isBlank(shopTypeJSONList)) {
        List<ShopType> shopTypes = JSONUtil.toList(shopTypeJSONList, ShopType.class);
        if(shopTypes.isEmpty()) {
            return Result.fail("没有类型");
        }
    ...
    if(shopTypes.isEmpty()) {
        return Result.fail("没有shopTypes");
    }
    //不要用for循环一个一个，写入，这样会覆盖。必须直接set整个shopTypes
...
**内存淘汰：**redis自动进行，当redis内存达到咱们设定的max-memery的时候，会自动触发淘汰机制，淘汰掉一些不重要的数据(可以自己设置策略方式) 。但是如果内存充足，就一直不会淘汰，这样如果存在旧数据，就一直不会被清除掉，所以一致性很差。
...
应**先操作数据库，再删除缓存**，如果先删除缓存，在两个线程并发来访问时，假设线程1先来，他先把缓存删了，此时线程2过来，他查询缓存数据并不存在,于是查数据库并写入缓存（此时他写入缓存这时缓存中的数据依然是旧的，因为数据库没更新）。然后线程1再更新数据库。

```java
//先删除缓存  后操作数据库的异常情况：
任务A:更新数据（先删缓存，再更新数据库）
任务B:查询数据（查缓存，缓存没有再去查数据库，然后写入缓存）
A {
	删除缓存
	B {
		查询缓存发现不存在
...
此时缓存中是旧值，数据库中是新值
先删缓存再更新数据库，删缓存很快，但是更新数据库很慢。所以别的线程很容易乘虚而入。
```

```java
//先操作数据库 后删除缓存的异常情况
任务A:查询数据（先查数据，缓存没有就查询数据库，然后写入缓存）
任务B:更新数据（先更新数据库，再删除缓存）
A {
    查询缓存->未命中->查询数据库(旧值)
    B {
        更新数据库(新值)
...
我们如果没有用CountDownLatch ，那么由于程序是异步的，当异步程序没有执行完时，主线程就已经执行完了，然后我们期望的是分线程全部走完之后，主线程再走，所以我们此时需要使用到CountDownLatch
...
但是这样添加锁，锁的粒度太粗了，在使用锁过程中，控制**锁粒度** 是一个非常重要的事情，因为如果锁的粒度太大，会导致每个线程进来都会锁住，所以我们需要去控制锁的粒度，**不要锁方法，而是锁住userId，因为我们希望不同的userId是不受到锁的影响的。**以下这段代码需要修改为：
...
持有锁的线程**在锁的内部出现了阻塞**，导致他的锁自动释放，这时其他线程，线程2来尝试获得锁，就拿到了这把锁，然后线程2在持有锁执行过程中，线程1反应过来，继续执行，而线程1执行过程中，走到了删除锁逻辑，此时就会把本应该属于线程2的锁进行删除，这就是误删别人锁的情况说明
...
```java
//同一个线程的UUID是相同的，所以相同JVM的相同线程在Redis中的value一定是相同的。
private static final String ID_PREFIX = UUID.randomUUID().toString(true) + "-";

public boolean tryLock(long timeoutSec) {
    Long threadId = Thread.currentThread().getId();
    String newThreadId = ID_PREFIX + threadId;  //更加健壮的线程标识。不同线程之间也不会冲突了
    Boolean success = stringRedisTemplate.opsForValue()
...
![image-20231222092205817](./assets/image-20231222092205817.png)
...
当我们去设置了多个锁时，redission会将多个锁添加到一个集合中，然后用while循环去不停去尝试拿锁，但是会有一个总共的加锁时间，这个时间是用需要加锁的个数 * 1500ms ，假设有3个锁，那么时间就是4500ms，假设在这4500ms内，所有的锁都加锁成功， 那么此时才算是加锁成功，如果在4500ms有线程加锁失败，则会再次去进行重试。这样如果出现其中一个节点进行主从同步是过程宕机了，导致没有同步成功，那么新的那个主节点，也是得不到锁的。于是就不会出现锁失效的问题。
